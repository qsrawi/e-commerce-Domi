...
export class ItemsViewComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public pageService: PageTypeService,
  ) {
  }
 isMobileDevice() {
    var check = false;
  ...
  ngOnInit(): void {
    console.log("__________________________________________"+this.offcanvas);
    ...
    this.listOptionsForm.get('limit')?.valueChanges.subscribe(value => {
      console.log("valueChangeslimit");
      this.TF = true;
    ...
    this.pageService.list$.pipe(
      filter((x): x is ItemsList => x !== null),
